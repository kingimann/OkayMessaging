...
enum ContactSyncStatus {
  ...
  /// Not available on this platform (web / desktop).
  unsupported,
...
class ContactsSync {
  ...
  /// The country calling code to assume for a bare local number, inferred from
  /// the signed-in user's own number (defaults to NANP "1").
  static String defaultCountryCode() {
    final phone = Session.instance.user.value?.phone ?? '';
    final digits = phone.replaceAll(RegExp(r'\D'), '');
    // NANP: 11 digits starting with 1 (e.g. 1 555 012 3456).
